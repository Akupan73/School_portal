...
app.post('/api/messages/:id/read', authenticate, async (req, res) => {
  ...
  if (msg.to !== target) return res.status(403).json({ error: 'Forbidden' })
  msg.read = true
  await db.write()
  res.json({ success: true })
})

// mark all messages for the authenticated user as read
app.post('/api/messages/mark-all-read', authenticate, async (req, res) => {
  await db.read()
  const target = req.session.role === 'admin' ? 'Admin' : req.session.email
  db.data.messages.forEach(m => { if (m.to === target) m.read = true })
  await db.write()
  res.json({ success: true })
})
